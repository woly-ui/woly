...
import { Field, Input, InputPassword, Toggle, Checkbox, TextArea } from 'ui';
...
<Playground size="S">
  <Field label='Label of set' isRow>
    <Checkbox text="Item text" variant="default" />
    <Checkbox text="Item text" variant="default" />
    <Checkbox text="Item text" variant="default" />
  </Field>
  <State initial={false} change={(i) => !i}>
    {(value, change) => (
      <Field label='Label of item' isRow>
        <Toggle
          isChecked={value}
          onChange={change}
          id="roundOne"
...
| `isRow`       | `boolean`             |             | The row attribute specifies direction of label and component     |
